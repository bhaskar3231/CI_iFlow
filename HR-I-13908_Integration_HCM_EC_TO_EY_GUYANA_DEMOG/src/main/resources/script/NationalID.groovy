...
xmlParser.'**'.findAll { node ->
    node.name() == 'PerNationalId' && node.cardType.text() == 'ID'
}.each { node ->
...
def String TinNumber(String tinNumber){
	 
def xmlParser = new XmlSlurper().parseText(tinNumber)

// Find all PerNationalId nodes, check cardType, print nationalId if cardType == 'ID'
def ids = []
xmlParser.'**'.findAll { node ->
    node.name() == 'PerNationalId' && node.cardType.text() == 'TIN'
}.each { node ->
    ids << node.nationalId.text()
}
return ids
