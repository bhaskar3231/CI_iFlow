    ...
    <xsl:for-each-group select="/EmpEmployment/EmpEmployment" group-by="personIdExternal">
      <Group>
         <xsl:for-each select="current-group()">
                   <xsl:sort select="jobInfoNav/EmpJob/startDate" order="descending"/>
		       <EmpEmployment>
                  ...
                  <userId><xsl:value-of select="jobInfoNav/EmpJob/userId"/></userId>
                  <startDate><xsl:value-of select="jobInfoNav/EmpJob/startDate"/></startDate>		       
		       </EmpEmployment>
	       </xsl:for-each>
      </Group>
    </xsl:for-each-group>
